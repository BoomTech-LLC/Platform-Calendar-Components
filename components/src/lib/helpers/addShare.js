...
export function downloadSharer(e, type, event) {
	...
	let desc = `
        ${
			event.desc
				? `${event.desc.replace(/&lt/g, '<').replace(/&gt/g, '>').replace(/&nbsp/g, ' ')}  `
				: ''
		}
        ${
	...
	)}&start=${event.start}&end=${event.end}&address=${encodeURIComponent(venue.address)}`;
		...
		case 'google':
			...
			if (event.all_day)
				url =
					...
					'&dates=' +
					moment(formatForAddtoCalendar(event, 'start', type)).format('YYYYMMDD') +
					'/' +
					moment(formatForAddtoCalendar(event, 'end')).format('YYYYMMDD') +
					'&details=' +
				...
				url =
					...
					encodeURIComponent(event.title) +
					'&dates=' +
					moment(formatForAddtoCalendar(event, 'start', type)).format(
						'YYYYMMDD[T]HHmmss'
					) +
					'/' +
					moment(formatForAddtoCalendar(event, 'end')).format('YYYYMMDD[T]HHmmss') +
					'&details=' +
					(event.desc ? eventDescription : '') +
					'&location=' +
					setLocation(event, 'encode') +
		...
		case 'yahoo':
			...
			if (event.all_day)
				url =
					...
					moment(formatForAddtoCalendar(event, 'start', type)).format('YYYYMMDD') +
					'&ET=' +
					moment(formatForAddtoCalendar(event, 'end', 'yahoo')).format('YYYYMMDD') +
					'&DESC=' +
					eventDescription +
					'&in_loc=' +
					setLocation(event);
...
function formatForAddtoCalendar(event, key, type) {
	...
	if (type === 'yahoo' && event.end !== event.start)
		fullEnd = moment(fullEnd).subtract(1, 'day').format('YYYY-MM-DDTHH:mm:ss');
...
const createDesc = (event, type) => {
	...
	return `${
		event.desc
			? `${encodeURIComponent(type === 'yahoo' ? plainTextFromHTML(event.desc) : event.desc)}`
			: ''
	}
...
export function generateEventUrl(event, boomEventUrlBase, comp_id) {
	if (event.kind === 4) {
		...
		return `${boomEventUrlBase}?cid=${comp_id}&eventId=${decodeId(`${event.id}`)}${
			event?.repeat?.type || event?.repeated ? '&startDate=' + event.start.split('T')[0] : ''
		}`;
	}
}
