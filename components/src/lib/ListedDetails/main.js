...
const ListedDetails = ({
  ...
  const hasAcceptableValues = Object.entries(parsedValues).some(
    ([key, value]) => LISTED_DETAILS_CONSTRUCTOR[key] && value
  );

  if (!values || isObjectEmpty(parsedValues) || !hasAcceptableValues)
    return null;

  return (
    <div
      className={combineClassNames([
        styles.listed_details_block,
        ...wrapperCustomClassNames,
      ])}
      style={{ gap: rowSpace }}>
